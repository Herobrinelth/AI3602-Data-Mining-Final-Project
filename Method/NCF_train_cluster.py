...
if __name__ == "__main__":
    epoches = Parameter.ncfclu_config["epoches"]
    batch_size = Parameter.ncfclu_config["batch_size"]
    device = 'cuda' if torch.cuda.is_available() else 'cpu'
    print("Running on {}".format(device))
    kwargs = Parameter.ncfclu_config["kwargs"]
    lr = Parameter.ncf_config["lr"]
    train_dataset = ML1m(Parameter.train_cluster)
    test_dataset = ML1m(Parameter.test_cluster)
    ...
    for e in range(epoches):
        ...
        if value_auc > best_auc:
            torch.save(model, Parameter.model_cluster)
            print("Best case updated, save to {}".format(Parameter.model_cluster))
            best_auc = value_auc
            continue
        count += 1
        if count == Parameter.early_stop:
            ...
            break
    Plot_errors(train_error, test_error, os.path.join(Parameter.output_root, Parameter.dataset), 'train_NCF_cluster.jpg', 'test_NCF_cluster.jpg')