...
if __name__ == "__main__":
    ...
    model = LSTM(device=device, feature_dims=train_dataset.feature_dims,
    ...
    for e in range(epoches):
        ...
        if value_auc > best_auc:
            ...
            print("Best case updated, save to {}".format(Parameter.lstm_path))
            best_auc = value_auc
            continue
        count += 1
        if count == Parameter.early_stop:
            ...
            break
    Plot_errors(train_error, test_error, os.path.join(Parameter.output_root, Parameter.dataset), 'train_LSTM.jpg', 'test_LSTM.jpg')