...
class SimItem:
    def __init__(self, train_rating, pretrain=False):
        ...
        if pretrain == True:
            with open(Parameter.movie_similarity_matrices_path, 'rb') as f:
    ...
    def FindSignature(self):
        ...
        with open(Parameter.movie_signature_path, 'wb') as f:
    ...
    def CompareSignature(self):
        ...
        with open(Parameter.movie_similarity_matrices_path, 'wb') as f:
...
if __name__ == '__main__':
    ...
    sit = SimItem(train_rating, False)
    accurate = 0
    recall = 0
    for user in tqdm(test_rating.keys()):
        ...
        if total != 0:
            accurate += (hit / total)
        recall += (hit / 10)
    with open(os.path.join(Parameter.output_root, Parameter.dataset, "output_hash.txt"), "w") as file:
