...
if __name__ == "__main__":
    train_rating = Dataset.LoadRatingDataset(Parameter.train_cluster)
    test_rating = Dataset.LoadRatingDataset(Parameter.test_cluster)
    modelpath = Parameter.model_cluster
    ...
    for user in tqdm(test_rating.keys()):
        ...
        if total != 0:
            accurate += (hit/total)
        recall += (hit/100)
    with open(os.path.join(Parameter.output_root, Parameter.dataset, "output_NCF_cluster.txt"), "w") as file:
